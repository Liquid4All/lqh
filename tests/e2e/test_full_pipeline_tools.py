"""E2E test: Full pipeline — seeded tool-calling data, train on remote

Usage:
    python -m tests.e2e.test_full_pipeline_tools
    python -m tests.e2e.test_full_pipeline_tools orchestration:2
    python -m tests.e2e.test_full_pipeline_tools orchestration:2 --timeout=600
"""
...
from tests.e2e.scenarios import FULL_PIPELINE_TOOLS
...
def _run_e2e(model: str, timeout: int):
    harness = E2EHarness(FULL_PIPELINE_TOOLS, orchestration_model=model)
    result = None
    start = time.time()
...
class TestFullPipelineToolsE2E(unittest.TestCase):
    def test_full_pipeline_tools(self) -> None:
        ...
        has_training = (
            ...
            or any(p.startswith("runs/") for p in result.artifacts)
        )
        self.assertTrue(has_training, "No training or eval was attempted (no start_training, start_local_eval, or runs/ artifacts)")
        ...
        async def _judge() -> None:
            ...
            judge_results = await judge_artifacts(client, FULL_PIPELINE_TOOLS, result.artifacts)
